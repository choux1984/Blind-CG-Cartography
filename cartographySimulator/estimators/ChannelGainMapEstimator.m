...
% 							s_check = [s_check(1:580,1);zeros(20,1);s_check(581:end,1)];
		...
		function [m_F_est,h_w_est] = blindEstimationOnlyEllipse(obj,t_sensorPos,t_sensorInd,v_measurements,v_measurementsNoShadowing)
			% Estimate a spatial loss field in blind fashion with a
			% chosen regularizer for the SLF(tikhonov, l-1, and total variation).
			% In addition, a weight function is estimated with a kernel
			% method. In particular, the weight function lies in
			...
			v_nz_PRKaIdx = ChannelGainMapEstimator.nzRelativeIdxFinder(v_nzWIdx,v_nzWbinSize,s_gridNum);
			m_K = ChannelGainMapEstimator.kernelMatrix(m_centroids,obj.h_kernel);
			...
			switch obj.ch_calibrationType
				case 'none'
					...
					while (estimateDifference > s_stoppingCriterion) && (s_iterationNum < s_iterationMax)
						...
						% [S1] estimate v_alpha
						m_IfPRK = obj.computeIfPRK(prev_v_f,v_nzWIdx,v_nzWbinSize,m_RK);			
						v_alpha = ChannelGainMapEstimator.ridgeRegression(m_IfPRK',s_check,obj.mu_w,m_K);
						
						% [S2] estimate v_f_est
						v_RKa = m_RK * v_alpha;
						v_tempA = zeros(s_gridNum * s_measurementNum,1);
						v_tempA(v_nz_PRKaIdx) = v_RKa;
				...
				case 'previous'
					...
					while (estimateDifference > s_stoppingCriterion) && (s_iterationNum < s_iterationMax)
						...
						m_IfPRK = obj.computeIfPRK(prev_v_f,v_nzWIdx,v_nzWbinSize,m_RK);	
						if strcmp(obj.ch_dataType,'real')
							m_IfPRK = m_IfPRK([1:580,601:2400],:);
						end		
						v_alpha = ChannelGainMapEstimator.ridgeRegression(m_IfPRK',s_check,obj.mu_w,m_K);
...
% 		end
		...
		function m_IfPRK = computeIfPRK(obj,v_f,v_nzWIdx,v_nzWbinSize,m_RK)
			...
			m_tempMat = m_pf .* m_RK;
			m_IfPRK(1,:) = sum(m_tempMat(1:v_nzWbinSize(1,1),:));
			for s_ItrIndex = 2 : size(v_nzWbinSize,1)
				s_lastPoint = sum(v_nzWbinSize(1:(s_ItrIndex-1),1));
				m_IfPRK(s_ItrIndex,:) = sum(m_tempMat(s_lastPoint+1:s_lastPoint + v_nzWbinSize(s_ItrIndex,1),:));
			end
...
% 			m_K = m_K + 10^8*eps* eye(s_centroidNum);
		...
		function v_nz_PRKaIdx = nzRelativeIdxFinder(v_nzWIdx,v_nzWbinSize,s_gridNum)
			...
			%
			% OUTPUT:
			%   v_nz_PRKaIdx         (s_measurementNum * s_gridNum) matrix
			%                        contains relative positions of nonzero
			%                        weights corresponding to R * m_Kernel
			%                        * v_alpha.
			
			v_nz_PRKaIdx = v_nzWIdx(1:v_nzWbinSize(1),1);
			for s_ItrIndex = 1 : size(v_nzWbinSize,1)-1
				s_lastPoint = sum(v_nzWbinSize(1:s_ItrIndex,1));
				v_nz_PRKaIdx =[ v_nz_PRKaIdx; v_nzWIdx(s_lastPoint+1 : s_lastPoint + v_nzWbinSize(s_ItrIndex+1),1) + s_ItrIndex * s_gridNum];		
			end
