function [hat_alpha,hat_f] = blind_est(K,idx_phi,s_check,mu_w,ini_f,eps_error,max_itr,mu)
...
%     TR = myiCovMat(sqrt(Ng),sqrt(Ng),var_shadow);
    ...
    for cnt = 1: max_itr
        ...
        %[S2]
%         [hat_f,A_K] = est_field(K,idx_phi,hat_alpha,s_check,Ng,TR,mu,RK);
%         [hat_f,A_K] = est_field_sp(K,idx_phi,hat_alpha,s_check,Ng,prev_hat_f,mu);
        prev_hat_f = hat_f;
        hat_f = est_field_ADMM(Ng,hat_alpha,mu,rho,num_itr,s_check,RK,D1,P,prev_hat_f);
